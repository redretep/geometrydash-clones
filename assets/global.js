...
function buildIcon(elements, current) {
	...
	loadIconLayers(iconConfig.form, iconConfig.id, function(a, b, c) {
		...
		new Icon(iconConfig, async function(icon) {
			...
			finishIcon(currentIcon, titleStr, dataURL, icon)
			if (overrideLoader) {
...
function finishIcon(currentIcon, name, dataURL, icon) {
	let iconImg = $(`<img title="${name}" style="${currentIcon.attr("imgStyle") || ""}" src="${dataURL}">`)
	currentIcon.attr("rendered", "true")
	currentIcon.removeAttr("rendering")
	if (currentIcon.attr("link")) {
		let params = [`id=${icon.id}`, `col1=${currentIcon.attr('col1')}`, `col2=${currentIcon.attr('col2')}`]
		if (icon.form != "player") params.splice(1, 0, `form=${icon.formName()}`)
		if (icon.glow) params.push("glow=1")
		if (icon.colors["g"] != icon.colors["2"]) params.push(`colG=${currentIcon.attr('colG')}`)

		let iconLink = $(`<a class="gdButton" target="_blank" href="/iconkit?${params.join('&')}"></a>`)
		iconLink.append(iconImg)
		currentIcon.append(iconLink)
	}
	else currentIcon.append(iconImg)
}
